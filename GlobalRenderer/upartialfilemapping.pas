...
begin
  ...
  {$IFDEF MSWINDOWS}
  if FFileHandle <> INVALID_HANDLE_VALUE then Close;
  {$ELSE}
  if FFD >= 0 then Close;
  {$ENDIF}
  FFileName := AValue;
...
begin
  ...
  wName := UTF8Decode(FFileName);
  FFileHandle := CreateFileW(PWideChar(wName), access, share, nil, disp, FILE_ATTRIBUTE_NORMAL, 0);
  if FFileHandle = INVALID_HANDLE_VALUE then
  begin
    err := GetLastError;
